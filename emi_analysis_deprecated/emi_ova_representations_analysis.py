...
#%%
emi_ova_reps = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\emi_ova_reps.csv", header = 0, index_col = 0)
emi_ova_mutations_biophys = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\emi_ova_mutations_biophys.csv", header = 0, index_col = None)
emi_ova_isolated_clones_reps = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\emi_isolated_clones_reps.csv", header = 0, index_col = 0)
emi_ova_isolated_clones_mutations_biophys = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\emi_ova_isolated_clones_mutations_biophys.csv", header = 0, index_col = None)
emi_ova_isolated_clones_binding = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\emi_ova_isolated_clones_ova_binding.csv", header = 0, index_col = 0)
...
lda_transform = pd.DataFrame(lda.transform(emi_ova_reps))

emi_ova_isolated_clones_binding_transform = pd.DataFrame(lda.transform(emi_ova_isolated_clones_reps))
emi_ova_isolated_clones_binding_predict = lda.predict(emi_ova_isolated_clones_reps)
emi_ova_isolated_clones_binding['zeros'] = 0
ax = sns.swarmplot(emi_ova_isolated_clones_binding.loc[:,'zeros'], emi_ova_isolated_clones_binding_transform.iloc[:,0], hue = emi_ova_isolated_clones_binding.iloc[:,2]/emi_ova_isolated_clones_binding.iloc[:,3], palette = 'Greens', s = 8)
ax.legend_.remove()

#%%
lda_variables= pd.concat([lda_transform, emi_ova_mutations_biophys], axis = 1, ignore_index = True)
lda_corrmat = lda_variables.corr()
sns.heatmap(lda_corrmat)

print(sc.stats.spearmanr((emi_ova_isolated_clones_binding.iloc[:,2]/emi_ova_isolated_clones_binding.iloc[:,3]), emi_ova_isolated_clones_binding_transform.iloc[:,0]))
plt.scatter((emi_ova_isolated_clones_binding.iloc[:,1]), emi_ova_isolated_clones_binding_transform.iloc[:,0])
...
rot_animation = animation.FuncAnimation(fig, rotate, frames=np.arange(0,360,4),interval=100)
rot_animation.save('emi_ova_tsne_ovalabels_color.gif', dpi=80, writer='imagemagick')

#%%
emi_ova_embedded.columns = ['t-SNE 1','t-SNE 2','t-SNE 3']
emi_ova_embedded.index = emi_ova_mutations_biophys.index
emi_ova_labels_l = list(emi_ova_labels.iloc[:,0])
...
for i in h33_mut:
    ...
    for j in emi_ova_mutations_biophys.index:
        if emi_ova_mutations_biophys.loc[j, '33HM'] == i:
            ...
            col.append(emi_ova_mutations_biophys.iloc[j, 43])
            ova_labs.append(emi_ova_labels_l[j])
...
ax[0][0].scatter(x_data[0], z_data[0], c = emi_ova_labels_const_hcdr1[0], edgecolors = 'black')
ax[0][0].set_title('Phenylalanine')
ax[0][1].scatter(x_data[1], z_data[1], c = emi_ova_labels_const_hcdr1[1], edgecolors = 'black')
ax[0][1].set_title('Tyrosine')
ax[0][2].scatter(x_data[2], z_data[2], c = emi_ova_labels_const_hcdr1[2], edgecolors = 'black')
ax[0][2].set_title('Serine')
ax[1][0].scatter(x_data[3], z_data[3], c = emi_ova_labels_const_hcdr1[3], edgecolors = 'black')
ax[1][0].set_title('Valine')
ax[1][1].scatter(x_data[4], z_data[4], c = emi_ova_labels_const_hcdr1[4], edgecolors = 'black')
ax[1][1].set_title('Alanine')
ax[1][2].scatter(x_data[5], z_data[5], c = emi_ova_labels_const_hcdr1[5], edgecolors = 'black')
...
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
scatter = ax.scatter(emi_ova_pca.iloc[:,0], emi_ova_pca.iloc[:,1], emi_ova_pca.iloc[:,2], c = emi_ova_labels.iloc[:,0])
ax.xaxis.pane.fill = False
ax.yaxis.pane.fill = False
ax.zaxis.pane.fill = False
legend1 = ax.legend(*scatter.legend_elements())
...
rot_animation = animation.FuncAnimation(fig, rotate, frames=np.arange(0,360,4),interval=100)
rot_animation.save('emi_ova_pca_mutations_ovalabels_color.gif', dpi=80, writer='imagemagick')

#%%
emi_ova_pca.index = emi_ova_mutations_biophys.index
emi_ova_variables = pd.concat([emi_ova_mutations_biophys, emi_ova_pca], axis = 1, ignore_index = True)
emi_corrmat = (emi_ova_variables).corr()
ind = list(emi_ova_mutations_biophys.columns)
ind.append('PCA1')
ind.append('PCA2')
ind.append('PCA3')
emi_corrmat.columns = ind
plt.figure()
sns.heatmap(emi_corrmat.iloc[:,56:59], )
...
sns.heatmap(emi_svc_corrmat.iloc[:,56:61])
...
fig = plt.figure(1)
ax = fig.add_subplot(111, projection='3d')
scatter = ax.scatter(emi_ova_embedded_selectfeat.iloc[:,0], emi_ova_embedded_selectfeat.iloc[:,1], emi_ova_embedded_selectfeat.iloc[:,2], c = emi_ova_mutations_biophys.iloc[:,41])
