...
#%%
emi_biophys = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\emi_biophys_0NotY.csv", header = 0, index_col = None)
emi_labels = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\emi_rep_labels_0NotY.csv", header = 0, index_col = 0)
...
emi_iso_ant_transforms_omit = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Emily Code\\emi_iso_ant_transforms_WTconst.csv", header = 0, index_col = 0)
emi_iso_psy_transforms_omit = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Emily Code\\emi_iso_psy_transforms_WTconst.csv", header = 0, index_col = 0)


emi_iso_seqs_0Y = []
emi_iso_biophys_0Y = []
emi_iso_binding_0Y = []
emi_iso_biophys_0NotY = []
...
for index, row in emi_iso_seqs.iterrows():
    ...
    if char[32] != 'Y':
        ...
        emi_iso_biophys_0NotY.append(emi_iso_biophys.loc[index,:])
...
emi_iso_biophys_0NotY = pd.DataFrame(emi_iso_biophys_0NotY)
...
emi_iso_ant_transform_0YLDA_all = pd.DataFrame(-1*(emi_ant.transform(emi_iso_biophys)))
emi_iso_ant_transform_0Y = pd.DataFrame(-1*(emi_ant.transform(emi_iso_biophys_0Y)))
emi_iso_ant_transform_0NotY = pd.DataFrame(-1*(emi_ant.transform(emi_iso_biophys_0NotY)))

emi_iso_ant_predict_0Y = pd.DataFrame(emi_ant.predict(emi_iso_biophys_0Y))
emi_iso_ant_predict_0NotY = pd.DataFrame(emi_ant.predict(emi_iso_biophys_0NotY))
...
#%%
emi_iso_psy_transform_0YLDA_all = pd.DataFrame(emi_psy.transform(emi_iso_biophys))
emi_iso_psy_transform_0Y = pd.DataFrame(emi_psy.transform(emi_iso_biophys_0Y))
emi_iso_psy_transform_0NotY = pd.DataFrame(emi_psy.transform(emi_iso_biophys_0NotY))

emi_iso_psy_predict_0Y = pd.DataFrame(emi_psy.predict(emi_iso_biophys_0Y))
emi_iso_psy_predict_0NotY = pd.DataFrame(emi_psy.predict(emi_iso_biophys_0NotY))
...
emi_iso_ant_transforms = pd.concat([emi_iso_ant_transforms, emi_iso_ant_transform_0Y.iloc[:,0]], axis = 1, ignore_index = False)
emi_iso_psy_transforms = pd.concat([emi_iso_psy_transforms, emi_iso_psy_transform_0Y.iloc[:,0]], axis = 1, ignore_index = False)

emi_iso_ant_transforms.to_csv('emi_iso_ant_transforms.csv', header = ['All Mutations','LO 0'], index = True)
emi_iso_psy_transforms.to_csv('emi_iso_psy_transforms.csv', header = ['All Mutations','LO 0'], index = True)
