...
#%%
emi_reps = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\emi_reps_3NotR.csv", header = 0, index_col = 0)
emi_labels = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\emi_rep_labels_3NotR.csv", header = 0, index_col = 0)
...
for index, row in emi_iso_seqs.iterrows():
    ...
    if char[55] != 'R':
        ...
        emi_iso_binding_3NotR.append(emi_iso_binding.loc[index,:])
emi_iso_seqs_3R = pd.DataFrame(emi_iso_seqs_3R)
emi_iso_seqs_3NotR = pd.DataFrame(emi_iso_seqs_3NotR)
emi_iso_reps_3R = pd.DataFrame(emi_iso_reps_3R)
emi_iso_reps_3NotR = pd.DataFrame(emi_iso_reps_3NotR)
emi_iso_binding_3R = pd.DataFrame(emi_iso_binding_3R)
emi_iso_binding_3NotR = pd.DataFrame(emi_iso_binding_3NotR)
...
emi_zero_rep = pd.DataFrame(emi_iso_reps.iloc[70,:]).T
emi_zero_binding = pd.DataFrame([emi_iso_binding.iloc[70,1:3]]).T
...
emi_iso_ant_transform_3RLDA_all = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps)))
emi_iso_ant_transform_3R = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps_3R)))
emi_iso_ant_transform_3NotR = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps_3NotR)))
emi_fit_ant_transform = pd.DataFrame(-1*(emi_ant.transform(emi_fit_reps)))
emi_iso_ant_predict_3R = pd.DataFrame(emi_ant.predict(emi_iso_reps_3R))
emi_iso_ant_predict_3NotR = pd.DataFrame(emi_ant.predict(emi_iso_reps_3NotR))
#emi_fit_ant_predict = pd.DataFrame(emi_ant.predict(emi_fit_reps))
print(stats.spearmanr(emi_iso_ant_transform_3RLDA_all.iloc[:,0], emi_iso_binding.iloc[:,1]))
print(stats.spearmanr(emi_iso_ant_transform_3R.iloc[:,0], emi_iso_binding_3R.iloc[:,1]))
print(stats.spearmanr(emi_iso_ant_transform_3NotR.iloc[:,0], emi_iso_binding_3NotR.iloc[:,1]))
print(accuracy_score(emi_iso_ant_predict_3NotR.iloc[:,0], emi_iso_binding_3NotR.iloc[:,3]))
...
emi_iso_ant_transform_3R['Fraction ANT Binding'] = ((emi_iso_ant_transform_3R.iloc[:,0]*x1[0])+x1[1])
emi_iso_ant_transform_3NotR['Fraction ANT Binding'] = ((emi_iso_ant_transform_3NotR.iloc[:,0]*x1[0])+x1[1])
emi_fit_ant_transform['Fraction ANT Binding'] = ((emi_fit_ant_transform.iloc[:,0]*x1[0])+x1[1])

plt.figure(1)
plt.scatter(emi_iso_ant_transform_3NotR.iloc[:,0], emi_iso_binding_3NotR.iloc[:,1], c = emi_iso_ant_predict_3NotR.iloc[:,0], cmap = cmap3, edgecolor = 'k', s = 75)
plt.scatter(emi_iso_ant_transform_3R.iloc[:,0], emi_iso_binding_3R.iloc[:,1], c = 'k', edgecolor = 'k', s = 75)
...
emi_iso_psy_transform_3RLDA_all = pd.DataFrame(emi_psy.transform(emi_iso_reps))
emi_iso_psy_transform_3R = pd.DataFrame(emi_psy.transform(emi_iso_reps_3R))
emi_iso_psy_transform_3NotR = pd.DataFrame(emi_psy.transform(emi_iso_reps_3NotR))
emi_fit_psy_transform = pd.DataFrame(emi_psy.transform(emi_fit_reps))
emi_iso_psy_predict_3R = pd.DataFrame(emi_psy.predict(emi_iso_reps_3R))
emi_iso_psy_predict_3NotR = pd.DataFrame(emi_psy.predict(emi_iso_reps_3NotR))
#emi_fit_psy_predict = pd.DataFrame(emi_psy.predict(emi_fit_reps))
print(stats.spearmanr(emi_iso_psy_transform_3RLDA_all.iloc[:,0], emi_iso_binding.iloc[:,1]))
print(stats.spearmanr(emi_iso_psy_transform_3R.iloc[:,0], emi_iso_binding_3R.iloc[:,1]))
print(stats.spearmanr(emi_iso_psy_transform_3NotR.iloc[:,0], emi_iso_binding_3NotR.iloc[:,1]))
print(accuracy_score(emi_iso_psy_predict_3NotR.iloc[:,0], emi_iso_binding_3NotR.iloc[:,4]))
...
emi_iso_psy_transform_3R['Fraction PSY Binding'] = ((emi_iso_psy_transform_3R.iloc[:,0]*x2[0])+x2[1])
emi_iso_psy_transform_3NotR['Fraction PSY Binding'] = ((emi_iso_psy_transform_3NotR.iloc[:,0]*x2[0])+x2[1])
emi_fit_psy_transform['Fraction PSY Binding'] = ((emi_fit_psy_transform.iloc[:,0]*x2[0])+x2[1])

plt.figure(3)
plt.scatter(emi_iso_psy_transform_3NotR.iloc[:,0], emi_iso_binding_3NotR.iloc[:,2], c = emi_iso_psy_predict_3NotR.iloc[:,0], cmap = cmap1, edgecolor = 'k', s = 75)
plt.scatter(emi_iso_psy_transform_3R.iloc[:,0], emi_iso_binding_3R.iloc[:,2], c = 'k', edgecolor = 'k', s = 75)
...
emi_iso_ant_transforms = pd.concat([emi_iso_ant_transforms, emi_iso_ant_transform_3R.iloc[:,0]], axis = 1, ignore_index = False)
emi_iso_psy_transforms = pd.concat([emi_iso_psy_transforms, emi_iso_psy_transform_3R.iloc[:,0]], axis = 1, ignore_index = False)

emi_iso_ant_transforms.to_csv('emi_iso_ant_transforms.csv', header = ['All Mutations','LO 0', 'LO 1', 'LO 2', 'LO 3'], index = True)
emi_iso_psy_transforms.to_csv('emi_iso_psy_transforms.csv', header = ['All Mutations','LO 0', 'Lo 1', 'LO 2', 'LO 3'], index = True)
