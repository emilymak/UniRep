...
#%%
emi_reps = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\emi_reps_5NotA.csv", header = 0, index_col = 0)
emi_labels = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\emi_rep_labels_5NotA.csv", header = 0, index_col = 0)
...
emi_iso_biophys = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\emi_iso_biophys_reduced.csv", header = 0, index_col = 0)
...
fig, ax = plt.subplots(1, figsize = (4.75,4.25))
ax.scatter(emi_iso_ant_transform_5NotA.iloc[:,0], emi_iso_binding_5NotA.iloc[:,1], c = emi_iso_ant_predict_5NotA.iloc[:,0], cmap = cmap7, edgecolor = 'k', s = 65, zorder = 4)
plt.scatter(emi_iso_ant_transform_5A.iloc[:,0], emi_iso_binding_5A.iloc[:,1], c = 'white', edgecolor = 'k', s = 65)
...
legend = plt.legend(handles=[pos_gate_patch, neg_gate_patch], fontsize = 14)
ax.set_ylim(-0.1,2.10)
plt.tick_params(labelsize = 14)
plt.ylabel('Normalized Affinity', fontsize = 20)
plt.xlabel('Affinity Projection', fontsize = 20)
...
emi_iso_ant_transforms = pd.concat([emi_iso_ant_transforms, emi_iso_ant_transform_5A.iloc[:,0]], axis = 1, ignore_index = False)
emi_iso_psy_transforms = pd.concat([emi_iso_psy_transforms, emi_iso_psy_transform_5A.iloc[:,0]], axis = 1, ignore_index = False)

emi_iso_ant_transforms.to_csv('emi_iso_ant_transforms.csv', header = ['All Mutations','LO 0', 'LO 1', 'LO 2', 'LO 3', 'LO 4', 'LO 5'], index = True)
emi_iso_psy_transforms.to_csv('emi_iso_psy_transforms.csv', header = ['All Mutations','LO 0', 'Lo 1', 'LO 2', 'LO 3', 'LO 4', 'LO 5'], index = True)
