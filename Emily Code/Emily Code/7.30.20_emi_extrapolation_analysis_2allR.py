...
#%%
emi_reps = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\emi_reps_2NotR.csv", header = 0, index_col = 0)
emi_labels = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\emi_rep_labels_2NotR.csv", header = 0, index_col = 0)
...
emi_iso_seqs_2NotR = []
emi_iso_reps_2NotR = []
emi_iso_binding_2NotR = []
for index, row in emi_iso_seqs.iterrows():
    char = list(row[0])
    if char[54] == 'R':
        char = ''.join(str(i) for i in char)
        emi_iso_seqs_2R.append(char)
        emi_iso_reps_2R.append(emi_iso_reps.loc[index,:])
        emi_iso_binding_2R.append(emi_iso_binding.loc[index,:])
    if char[54] != 'R':
        char = ''.join(str(i) for i in char)
        emi_iso_seqs_2NotR.append(char)
        emi_iso_reps_2NotR.append(emi_iso_reps.loc[index,:])
        emi_iso_binding_2NotR.append(emi_iso_binding.loc[index,:])
emi_iso_seqs_2R = pd.DataFrame(emi_iso_seqs_2R)
emi_iso_seqs_2NotR = pd.DataFrame(emi_iso_seqs_2NotR)
emi_iso_reps_2R = pd.DataFrame(emi_iso_reps_2R)
emi_iso_reps_2NotR = pd.DataFrame(emi_iso_reps_2NotR)
...
emi_iso_ant_transform_2RLDA_all = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps)))
emi_iso_ant_transform_2R = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps_2R)))
emi_iso_ant_transform_2NotR = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps_2NotR)))
emi_fit_ant_transform = pd.DataFrame(-1*(emi_ant.transform(emi_fit_reps)))
emi_iso_ant_predict_2R = pd.DataFrame(emi_ant.predict(emi_iso_reps_2R))
emi_iso_ant_predict_2NotR = pd.DataFrame(emi_ant.predict(emi_iso_reps_2NotR))
#emi_fit_ant_predict = pd.DataFrame(emi_ant.predict(emi_fit_reps))
print(stats.spearmanr(emi_iso_ant_transform_2RLDA_all.iloc[:,0], emi_iso_binding.iloc[:,1]))
print(stats.spearmanr(emi_iso_ant_transform_2R.iloc[:,0], emi_iso_binding_2R.iloc[:,1]))
print(stats.spearmanr(emi_iso_ant_transform_2NotR.iloc[:,0], emi_iso_binding_2NotR.iloc[:,1]))
print(accuracy_score(emi_iso_ant_predict_2NotR.iloc[:,0], emi_iso_binding_2NotR.iloc[:,3]))
...
emi_iso_ant_transform_2R['Fraction ANT Binding'] = ((emi_iso_ant_transform_2R.iloc[:,0]*x1[0])+x1[1])
emi_iso_ant_transform_2NotR['Fraction ANT Binding'] = ((emi_iso_ant_transform_2NotR.iloc[:,0]*x1[0])+x1[1])
emi_fit_ant_transform['Fraction ANT Binding'] = ((emi_fit_ant_transform.iloc[:,0]*x1[0])+x1[1])

plt.figure(1)
plt.scatter(emi_iso_ant_transform_2NotR.iloc[:,0], emi_iso_binding_2NotR.iloc[:,1], c = emi_iso_ant_predict_2NotR.iloc[:,0], cmap = cmap3, edgecolor = 'k', s = 75)
plt.scatter(emi_iso_ant_transform_2R.iloc[:,0], emi_iso_binding_2R.iloc[:,1], c = 'k', edgecolor = 'k', s = 75)
...
y_patch = mpatches.Patch(facecolor='black', label = 'Sequence 54=R', edgecolor = 'black', linewidth = 0.5)
...
plt.title('Experimental Antigen Binding vs LDA Transform 54=R', fontsize = 17)
plt.tight_layout()
...
emi_iso_psy_transform_2RLDA_all = pd.DataFrame(emi_psy.transform(emi_iso_reps))
emi_iso_psy_transform_2R = pd.DataFrame(emi_psy.transform(emi_iso_reps_2R))
emi_iso_psy_transform_2NotR = pd.DataFrame(emi_psy.transform(emi_iso_reps_2NotR))
emi_fit_psy_transform = pd.DataFrame(emi_psy.transform(emi_fit_reps))
emi_iso_psy_predict_2R = pd.DataFrame(emi_psy.predict(emi_iso_reps_2R))
emi_iso_psy_predict_2NotR = pd.DataFrame(emi_psy.predict(emi_iso_reps_2NotR))
#emi_fit_psy_predict = pd.DataFrame(emi_psy.predict(emi_fit_reps))
print(stats.spearmanr(emi_iso_psy_transform_2RLDA_all.iloc[:,0], emi_iso_binding.iloc[:,1]))
print(stats.spearmanr(emi_iso_psy_transform_2R.iloc[:,0], emi_iso_binding_2R.iloc[:,1]))
print(stats.spearmanr(emi_iso_psy_transform_2NotR.iloc[:,0], emi_iso_binding_2NotR.iloc[:,1]))
print(accuracy_score(emi_iso_psy_predict_2NotR.iloc[:,0], emi_iso_binding_2NotR.iloc[:,4]))
...
emi_iso_psy_transform_2R['Fraction PSY Binding'] = ((emi_iso_psy_transform_2R.iloc[:,0]*x2[0])+x2[1])
emi_iso_psy_transform_2NotR['Fraction PSY Binding'] = ((emi_iso_psy_transform_2NotR.iloc[:,0]*x2[0])+x2[1])
emi_fit_psy_transform['Fraction PSY Binding'] = ((emi_fit_psy_transform.iloc[:,0]*x2[0])+x2[1])

plt.figure(3)
plt.scatter(emi_iso_psy_transform_2NotR.iloc[:,0], emi_iso_binding_2NotR.iloc[:,2], c = emi_iso_psy_predict_2NotR.iloc[:,0], cmap = cmap1, edgecolor = 'k', s = 75)
plt.scatter(emi_iso_psy_transform_2R.iloc[:,0], emi_iso_binding_2R.iloc[:,2], c = 'k', edgecolor = 'k', s = 75)
...
plt.xlabel('LDA Transform', fontsize = 16)
plt.title('Experimental PSY Binding vs LDA Transform 54=R', fontsize = 18)
plt.xlim(-5,5)
plt.ylim(0,1.35)
plt.tight_layout()
...
emi_iso_ant_transforms = pd.concat([emi_iso_ant_transforms, emi_iso_ant_transform_2R.iloc[:,0]], axis = 1, ignore_index = False)
emi_iso_psy_transforms = pd.concat([emi_iso_psy_transforms, emi_iso_psy_transform_2R.iloc[:,0]], axis = 1, ignore_index = False)

emi_iso_ant_transforms.to_csv('emi_iso_ant_transforms.csv', header = ['All Mutations', 'LO 0' ,'LO 1', 'LO2'], index = True)
emi_iso_psy_transforms.to_csv('emi_iso_psy_transforms.csv', header = ['All Mutations', 'LO 0' ,'LO 1', 'LO2'], index = True)
