...
#%%
emi_reps = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\emi_reps_1R.csv", header = 0, index_col = 0)
emi_labels = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\emi_rep_labels_1R.csv", header = 0, index_col = 0)
...
emi_iso_seqs_1NotR = []
emi_iso_reps_1NotR = []
emi_iso_binding_1NotR = []
for index, row in emi_iso_seqs.iterrows():
    char = list(row[0])
    if char[49] == 'R':
        char = ''.join(str(i) for i in char)
        emi_iso_seqs_1R.append(char)
        emi_iso_reps_1R.append(emi_iso_reps.loc[index,:])
        emi_iso_binding_1R.append(emi_iso_binding.loc[index,:])
    if char[49] != 'R':
        char = ''.join(str(i) for i in char)
        emi_iso_seqs_1NotR.append(char)
        emi_iso_reps_1NotR.append(emi_iso_reps.loc[index,:])
        emi_iso_binding_1NotR.append(emi_iso_binding.loc[index,:])
emi_iso_seqs_1R = pd.DataFrame(emi_iso_seqs_1R)
emi_iso_seqs_1NotR = pd.DataFrame(emi_iso_seqs_1NotR)
emi_iso_reps_1R = pd.DataFrame(emi_iso_reps_1R)
emi_iso_reps_1NotR = pd.DataFrame(emi_iso_reps_1NotR)
emi_iso_binding_1R = pd.DataFrame(emi_iso_binding_1R)
emi_iso_binding_1NotR = pd.DataFrame(emi_iso_binding_1NotR)
...
emi_zero_rep = pd.DataFrame(emi_iso_reps.iloc[83,:]).T
emi_zero_binding = pd.DataFrame([emi_iso_binding.iloc[83,1:3]]).T
...
emi_iso_ant_transform_1RLDA_all = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps)))
emi_iso_ant_transform_1R = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps_1R)))
emi_iso_ant_transform_1NotR = pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps_1NotR)))
emi_fit_ant_transform = pd.DataFrame(-1*(emi_ant.transform(emi_fit_reps)))
emi_iso_ant_predict_1R = pd.DataFrame(emi_ant.predict(emi_iso_reps_1R))
emi_iso_ant_predict_1NotR = pd.DataFrame(emi_ant.predict(emi_iso_reps_1NotR))
#emi_fit_ant_predict = pd.DataFrame(emi_ant.predict(emi_fit_reps))
print(stats.spearmanr(emi_iso_ant_transform_1RLDA_all.iloc[:,0], emi_iso_binding.iloc[:,1]))
print(stats.spearmanr(emi_iso_ant_transform_1R.iloc[:,0], emi_iso_binding_1R.iloc[:,1]))
print(stats.spearmanr(emi_iso_ant_transform_1NotR.iloc[:,0], emi_iso_binding_1NotR.iloc[:,1]))
print(accuracy_score(emi_iso_ant_predict_1NotR.iloc[:,0], emi_iso_binding_1NotR.iloc[:,3]))
...
emi_iso_ant_transform_1R['Fraction ANT Binding'] = ((emi_iso_ant_transform_1R.iloc[:,0]*x1[0])+x1[1])
emi_iso_ant_transform_1NotR['Fraction ANT Binding'] = ((emi_iso_ant_transform_1NotR.iloc[:,0]*x1[0])+x1[1])
emi_fit_ant_transform['Fraction ANT Binding'] = ((emi_fit_ant_transform.iloc[:,0]*x1[0])+x1[1])

plt.figure(1)
plt.scatter(emi_iso_ant_transform_1NotR.iloc[:,0], emi_iso_binding_1NotR.iloc[:,1], c = emi_iso_ant_predict_1NotR.iloc[:,0], cmap = cmap3, edgecolor = 'k', s = 75)
plt.scatter(emi_iso_ant_transform_1R.iloc[:,0], emi_iso_binding_1R.iloc[:,1], c = 'k', edgecolor = 'k', s = 75)
...
y_patch = mpatches.Patch(facecolor='black', label = 'Sequence 50=R', edgecolor = 'black', linewidth = 0.5)
...
plt.title('Experimental Antigen Binding vs LDA Transform 50=R', fontsize = 17)
plt.tight_layout()
...
emi_iso_psy_transform_1RLDA_all = pd.DataFrame(emi_psy.transform(emi_iso_reps))
emi_iso_psy_transform_1R = pd.DataFrame(emi_psy.transform(emi_iso_reps_1R))
emi_iso_psy_transform_1NotR = pd.DataFrame(emi_psy.transform(emi_iso_reps_1NotR))
emi_fit_psy_transform = pd.DataFrame(emi_psy.transform(emi_fit_reps))
emi_iso_psy_predict_1R = pd.DataFrame(emi_psy.predict(emi_iso_reps_1R))
emi_iso_psy_predict_1NotR = pd.DataFrame(emi_psy.predict(emi_iso_reps_1NotR))
#emi_fit_psy_predict = pd.DataFrame(emi_psy.predict(emi_fit_reps))
print(stats.spearmanr(emi_iso_psy_transform_1RLDA_all.iloc[:,0], emi_iso_binding.iloc[:,1]))
print(stats.spearmanr(emi_iso_psy_transform_1R.iloc[:,0], emi_iso_binding_1R.iloc[:,1]))
print(stats.spearmanr(emi_iso_psy_transform_1NotR.iloc[:,0], emi_iso_binding_1NotR.iloc[:,1]))
print(accuracy_score(emi_iso_psy_predict_1NotR.iloc[:,0], emi_iso_binding_1NotR.iloc[:,4]))
...
emi_iso_psy_transform_1R['Fraction PSY Binding'] = ((emi_iso_psy_transform_1R.iloc[:,0]*x2[0])+x2[1])
emi_iso_psy_transform_1NotR['Fraction PSY Binding'] = ((emi_iso_psy_transform_1NotR.iloc[:,0]*x2[0])+x2[1])
emi_fit_psy_transform['Fraction PSY Binding'] = ((emi_fit_psy_transform.iloc[:,0]*x2[0])+x2[1])

plt.figure(3)
plt.scatter(emi_iso_psy_transform_1NotR.iloc[:,0], emi_iso_binding_1NotR.iloc[:,2], c = emi_iso_psy_predict_1NotR.iloc[:,0], cmap = cmap1, edgecolor = 'k', s = 75)
plt.scatter(emi_iso_psy_transform_1R.iloc[:,0], emi_iso_binding_1R.iloc[:,2], c = 'k', edgecolor = 'k', s = 75)
...
plt.xlabel('LDA Transform', fontsize = 16)
plt.title('Experimental PSY Binding vs LDA Transform 50=R', fontsize = 18)
plt.xlim(-4.5,5)
plt.ylim(0,1.35)
plt.tight_layout()
...
#%%
"""
emi_iso_ant_transforms = pd.concat([emi_iso_ant_transforms, emi_iso_ant_transform_1RLDA_all.iloc[:,0]], axis = 1)
emi_iso_psy_transforms = pd.concat([emi_iso_psy_transforms, emi_iso_psy_transform_1RLDA_all.iloc[:,0]], axis = 1)

emi_iso_ant_transforms.to_csv('emi_iso_ant_transforms.csv', header = ['All Mutations', 'LO 7', 'LO 6', 'LO 5', 'LO 4', 'LO 3', 'LO 2', 'LO 1'], index = True)
emi_iso_psy_transforms.to_csv('emi_iso_psy_transforms.csv', header = ['All Mutations', 'LO 7', 'LO 6', 'LO 5', 'LO 4', 'LO 3', 'LO 2', 'LO 1'], index = True)
