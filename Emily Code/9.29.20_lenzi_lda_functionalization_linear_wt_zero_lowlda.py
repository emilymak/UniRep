...
lenzi_seqs = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\lenzi_seqs.csv", header = 0, index_col = 0)
lenzi_wt_seq = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\lenzi_wt_seq.csv", header = None)
...
lenzi_IgG_seqs = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\seqs\\lenzi_IgG_seqs.csv", header = 0, index_col = 0)
lenzi_IgG_reps = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\lenzi_IgG_reps.csv", header = 0, index_col = 0)
lenzi_IgG_binding = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\lenzi_IgG_binding.csv", header = 0, index_col = 0)
...
lenzi_IgG_ant_transform = pd.DataFrame(-1*(lenzi_ant.transform(lenzi_IgG_reps)))
...
lenzi_IgG_psy_transform = pd.DataFrame(lenzi_psy.transform(lenzi_IgG_reps))
...
for index, row in lenzi_ant_transform.iterrows():
    if (lenzi_ant_transform.iloc[index,0] > 1.20) & (lenzi_psy_transform.iloc[index,0] < 1):
        clones_score[index] = 2
        lenzi_optimal_sequences.append([index, 2, lenzi_labels.iloc[index, 0]])
    if (lenzi_ant_transform.iloc[index,0] > 0.90) & (lenzi_psy_transform.iloc[index,0] < 0.85):
...
for index, row in lenzi_optimal_muts.iterrows():
    muts = list(row)
    wt_list = list(lenzi_wt_seq.iloc[0,0])
    ham = hamming(muts, wt_list)
...
ax00 = ax0.scatter(lenzi_ant_transform.iloc[:,0], lenzi_psy_transform.iloc[:,0], c = 'deepskyblue')
...
ax10 = ax1.scatter(lenzi_ant_transform.iloc[:,0], lenzi_psy_transform.iloc[:,0], c = 'deepskyblue')
...
ax2.scatter(lenzi_IgG_ant_transform.iloc[:,0], lenzi_IgG_psy_transform.iloc[:,0], c = 'white', s = 65, edgecolor = 'k')


#%%
print(sc.stats.spearmanr(lenzi_IgG_ant_transform.iloc[:,0], lenzi_IgG_binding.iloc[:,0]))
plt.scatter(lenzi_IgG_ant_transform.iloc[:,0], lenzi_IgG_binding.iloc[:,0])

print(sc.stats.spearmanr(lenzi_IgG_psy_transform.iloc[:,0], lenzi_IgG_binding.iloc[:,1]))
plt.scatter(lenzi_IgG_psy_transform.iloc[:,0], lenzi_IgG_binding.iloc[:,1])
