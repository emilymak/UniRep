# -*- coding: utf-8 -*-
"""
Created on Thu Oct 29 14:13:46 2020
...
#%%
emi_reps_ova = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\emi_reps_ova.csv", header = 0, index_col = 0)
emi_labels = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\emi_rep_labels_ova.csv", header = 0, index_col = 0)
#emi_biophys = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\emi_biophys.csv", header = 0, index_col = 0)

emi_iso_reps_ova = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\emi_iso_reps_reduced.csv", header = 0, index_col = 0)

emi_zero_rep = pd.DataFrame(emi_reps_ova.iloc[2945,:]).T
...
emi_fit_reps_ova = pd.concat([emi_wt_rep, emi_zero_rep])
...
emi_iso_seqs_ova = pd.read_csv("C:\\Users\\makow\\Documents\\GitHub\\UniRep\\Datasets\\emi_iso_seqs_reduced.csv", header = None)
emi_iso_seqs_ova.columns = ['Sequences']
...
emi_reps_ova_train, emi_reps_ova_test, emi_ant_train, emi_ant_test = train_test_split(emi_reps_ova, emi_labels.iloc[:,3])

emi_ant = LDA()
cv_lda = cv(emi_ant, emi_reps_ova, emi_labels.iloc[:,3], cv = 10)
print(np.mean(cv_lda['test_score']))

emi_ant_transform = pd.DataFrame(-1*(emi_ant.fit_transform(emi_reps_ova, emi_labels.iloc[:,3])))
emi_ant_predict = pd.DataFrame(emi_ant.predict(emi_reps_ova))
...
emi_iso_ant_transform= pd.DataFrame(-1*(emi_ant.transform(emi_iso_reps_ova)))
emi_fit_ant_transform= pd.DataFrame(-1*(emi_ant.transform(emi_fit_reps_ova)))
emi_iso_ant_predict = pd.DataFrame(emi_ant.predict(emi_iso_reps_ova))
emi_fit_ant_predict = pd.DataFrame(emi_ant.predict(emi_fit_reps_ova))
...
emi_reps_ova_train, emi_reps_ova_test, emi_psy_train, emi_psy_test = train_test_split(emi_reps_ova, emi_labels.iloc[:,2])

emi_psy = LDA()
cv_lda = cv(emi_psy, emi_reps_ova, emi_labels.iloc[:,2], cv = 10)
print(np.mean(cv_lda['test_score']))

emi_psy_transform = pd.DataFrame(emi_psy.fit_transform(emi_reps_ova, emi_labels.iloc[:,2]))
emi_psy_predict = pd.DataFrame(emi_psy.predict(emi_reps_ova))
...
emi_iso_psy_transform= pd.DataFrame(emi_psy.transform(emi_iso_reps_ova))
emi_fit_psy_transform= pd.DataFrame(emi_psy.transform(emi_fit_reps_ova))
emi_iso_psy_predict = pd.DataFrame(emi_psy.predict(emi_iso_reps_ova))
emi_fit_psy_predict = pd.DataFrame(emi_psy.predict(emi_fit_reps_ova))
...
ax2.scatter(emi_base_seq_transform.iloc[:,0], emi_base_seq_transform.iloc[:,1])
ax2.scatter(emi_novel_clones_transform_ant.iloc[:,0], emi_novel_clones_transform_psy.iloc[:,0])
