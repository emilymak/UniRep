# -*- coding: utf-8 -*-
"""
Created on Thu Oct  8 18:22:02 2020
...
colormap1 = np.array(['blue','darkgray'])
colormap1_r = np.array(['darkgray', 'blue'])
colormap1r = np.array(['red','darkgray'])
colormap1r_r = np.array(['darkgray', 'red'])
colormap2 = np.array(['royalblue', 'blue', 'navy', 'darkgray'])
colormap_br = np.array(['blue', 'white', 'red'])

colormap3 = np.array(['blue', 'darkgray'])
...
colormap_pareto = np.array(['darkgray', 'red', 'darkviolet', 'blue'])
cmap_pareto = LinearSegmentedColormap.from_list("mycmap", colormap_pareto)
cmap_br = LinearSegmentedColormap.from_list("mycmap", colormap_br)
...
ax.scatter(emi_ant_transform.iloc[:,0], emi_psy_transform.iloc[:,0], c = emi_biophys.iloc[:,63], edgecolor = 'k', linewidth = 0.1, cmap = cmap_br)
plt.scatter(emi_wt_ant_transform.iloc[:,0], emi_wt_psy_transform.iloc[:,0],  c = 'k', s = 65, edgecolor = 'k')
ax.tick_params(labelsize = 16)


#%%
fig, ax = plt.subplots(figsize = (5.5, 5.25))
ax.scatter(emi_ant_transform.iloc[:,0], emi_psy_transform.iloc[:,0], c = clones_score, cmap = cmap_pareto, edgecolor = 'k', linewidth = 0.1)
plt.scatter(emi_wt_ant_transform.iloc[:,0], emi_wt_psy_transform.iloc[:,0],  c = 'k', s = 65, edgecolor = 'k')
ax.tick_params(labelsize = 16)


#%%
affinity_proj = [-2.822544191, -2.46492529, -1.699226014, -1.240606051, -1.592595398, -1.898476205, -2.399348352, -2.868485042, -2.778255279, -2.814564258, -1.981005497, -2.968343676, -1.699366286, -2.527429587]
specificity_proj = [2.609735623, 2.143824778, 1.409232478, 0.962155947, 1.117217874, 1.402896019, 1.709838732, 1.601674534, 1.525003241, 1.854153249, 1.111641317, 1.921408569, 0.9798308, 2.102457594]
colors = [0, 2, 2, 1, 1, 2, 2, 3,2, 2, 2, 3, 2, 2]
fig, ax = plt.subplots(figsize = (5.5, 5.25))
ax.scatter(emi_ant_transform.iloc[:,0], emi_psy_transform.iloc[:,0], c = 'darkgray', edgecolor = 'k', linewidth = 0.1)
plt.scatter(affinity_proj, specificity_proj,  c = colors, cmap = cmap_pareto, s = 65, edgecolor = 'k')
plt.scatter(emi_wt_ant_transform.iloc[:,0], emi_wt_psy_transform.iloc[:,0],  c = 'k', s = 65, edgecolor = 'k')
ax.tick_params(labelsize = 16)
...
ax = sns.swarmplot(emi_labels.iloc[:,3], emi_ant_transform.iloc[:,0], hue = emi_labels.iloc[:,2], s = 4, edgecolor = 'k', linewidth = 0.05, palette = colormap1r)
...
#%%
fig, ax = plt.subplots(figsize=(11,6.5))
ax = sns.swarmplot(emi_labels.iloc[:,2], emi_psy_transform.iloc[:,0], hue = emi_labels.iloc[:,3], s = 4, edgecolor = 'k', linewidth = 0.05, palette = colormap1_r)
ax.tick_params(labelsize = 0)
ax.set_ylabel(' ', fontsize = 18)
ax.set_xticklabels([' ', ' '])
ax.set_xlabel('')
