...
emi_iso_binding_1R5NotA = []
emi_iso_seqs_1NotR5A = []
emi_iso_reps_1NotR5A = []
emi_iso_binding_1NotR5A = []
for index, row in emi_iso_seqs.iterrows():
    char = list(row[0])
    if (char[50] == 'R') & (char[98] != 'A'):
        char = ''.join(str(i) for i in char)
        emi_iso_seqs_1R5NotA.append(char)
        emi_iso_reps_1R5NotA.append(emi_iso_reps.loc[index,:])
        emi_iso_binding_1R5NotA.append(emi_iso_binding.loc[index,:])
    if (char[50] != 'R') & (char[98] == 'A'):
        char = ''.join(str(i) for i in char)
        emi_iso_seqs_1NotR5A.append(char)
        emi_iso_reps_1NotR5A.append(emi_iso_reps.loc[index,:])
        emi_iso_binding_1NotR5A.append(emi_iso_binding.loc[index,:])
emi_iso_seqs_1R5NotA = pd.DataFrame(emi_iso_seqs_1R5NotA)
emi_iso_seqs_1NotR5A = pd.DataFrame(emi_iso_seqs_1NotR5A)
emi_iso_reps_1R5NotA = pd.DataFrame(emi_iso_reps_1R5NotA)
emi_iso_reps_1NotR5A = pd.DataFrame(emi_iso_reps_1NotR5A)
emi_iso_binding_1R5NotA = pd.DataFrame(emi_iso_binding_1R5NotA)
emi_iso_binding_1NotR5A = pd.DataFrame(emi_iso_binding_1NotR5A)
