# -*- coding: utf-8 -*-
"""
Created on Thu Jun 18 22:09:32 2020
...
emi_iso_reps, emi_iso_reps_test, emi_iso_binding, emi_iso_binding_test = train_test_split(emi_iso_reps, emi_iso_binding)
emi_iso_binding.reset_index(inplace = True, drop = True)
emi_iso_binding_test.reset_index(inplace = True, drop = True)
emi_iso_reps.reset_index(inplace = True, drop = True)
emi_iso_reps_test.reset_index(inplace = True, drop = True)
...
emi_reps_stringent_train, emi_reps_stringent_test, emi_ant_stringent_train, emi_ant_stringent_test = train_test_split(emi_reps_stringent, emi_labels_stringent.iloc[:,3])

emi_ant_stringent = LDA()
cv_lda_stringent = cv(emi_ant_stringent, emi_reps_stringent, emi_labels_stringent.iloc[:,3], cv = 10)

emi_ant_transform_stringent = pd.DataFrame(-1*(emi_ant_stringent.fit_transform(emi_reps_stringent, emi_labels_stringent.iloc[:,3])))
emi_ant_predict_stringent = pd.DataFrame(emi_ant_stringent.predict(emi_reps_stringent))
print(confusion_matrix(emi_ant_predict_stringent.iloc[:,0], emi_labels_stringent.iloc[:,3]))

emi_wt_ant_transform = pd.DataFrame(-1*(emi_ant_stringent.transform(emi_wt_rep)))
...
emi_ant_iso_transform_stringent= pd.DataFrame(-1*(emi_ant_stringent.transform(emi_iso_reps)))
emi_ant_iso_transform_test_stringent= pd.DataFrame(-1*(emi_ant_stringent.transform(emi_iso_reps_test)))
emi_ant_iso_predict_stringent = pd.DataFrame(emi_ant_stringent.predict(emi_iso_reps))
...
emi_reps_stringent_train, emi_reps_stringent_test, emi_psy_stringent_train, emi_psy_stringent_test = train_test_split(emi_reps_stringent, emi_labels_stringent.iloc[:,2])

emi_psy_stringent = LDA()
cv_lda_stringent = cv(emi_psy_stringent, emi_reps_stringent, emi_labels_stringent.iloc[:,2], cv = 10)

emi_psy_transform_stringent = pd.DataFrame(emi_psy_stringent.fit_transform(emi_reps_stringent, emi_labels_stringent.iloc[:,2]))
emi_psy_predict_stringent = pd.DataFrame(emi_psy_stringent.predict(emi_reps_stringent))
print(confusion_matrix(emi_psy_predict_stringent.iloc[:,0], emi_labels_stringent.iloc[:,2]))

emi_wt_psy_transform = pd.DataFrame(emi_psy_stringent.transform(emi_wt_rep))
...
emi_psy_iso_transform_stringent= pd.DataFrame(emi_psy_stringent.transform(emi_iso_reps))
emi_psy_iso_transform_test_stringent= pd.DataFrame(emi_psy_stringent.transform(emi_iso_reps_test))
emi_psy_iso_predict_stringent = pd.DataFrame(emi_psy_stringent.predict(emi_iso_reps))
...
for index, row in emi_ant_transform_stringent.iterrows():
    ...
    if (emi_ant_transform_stringent.iloc[index,1] > 0.9) & (emi_psy_transform_stringent.iloc[index,1] < 0.6):
...
for index, row in emi_ant_iso_transform_stringent.iterrows():
    ...
    if (emi_ant_iso_transform_stringent.iloc[index,1] > 0.9) & (emi_psy_iso_transform_stringent.iloc[index,1] < 0.6):
...
for index, row in emi_iso_binding_test.iterrows():
    ...
    if (emi_ant_iso_transform_stringent.iloc[index,1] > 0.7):
...
fig, ax = plt.subplots(figsize = (7,4.5))
img = plt.scatter(emi_iso_binding_test.iloc[:,1], emi_iso_binding_test.iloc[:,2], c = iso_score_test_stringent, cmap = cmap4, edgecolor = 'k', s = 75, lw = 1.5, zorder = 2)
